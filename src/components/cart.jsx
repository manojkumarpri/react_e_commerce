...
class Cart extends Component {
  ...
  Remove(id) {
    ...
    for(var j=0;j<this.state.provider.length;j++){
      if(this.state.provider[j].prodId[this.state.provider[j].index]==id){
...
console.log(this.state.combine)
  ...
  checkout(a){
    ...
    for(var i=0;i<this.state.combine.length;i++){
      if(this.state.combine[i].product_id==a.product_id){
        this.state.combine.splice(i, 1);
      }
    }
    console.log(this.state.combine);
  ...
  render() {
    ...
    return (
      <div>
        ...
        <hr/>
        {this.state.combine!=undefined  && this.state.combine!=null ? (this.state.combine.map((a,index) =>
    ...
    <div>
       {this.props.history.push('/err')}
        ...
        </div>{this.state.combine!=null && this.state.show==true ?(
        <div>{this.state.combine.length!=0 ?(
        <button className="btn btn-success" onClick={()=>this.checkout1()}>Go to checkout</button>
         ):(this.props.history.push('/checkout'))} </div>):[]}
